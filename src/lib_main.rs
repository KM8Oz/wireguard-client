...
fn get_client_keys(input_path:&String) -> ClientConfig {
...
fn get_wg_server(input_path:&String) -> WG_server {
...
pub fn start(client_keys:&String, wg_server:&String){
   println!("Start wg client");
    let c_key_pair = get_client_keys(client_keys); // client
    let server = get_wg_server(wg_server);
